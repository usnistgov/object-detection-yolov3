...
TILE_SIZE = 512 + 96  # 96 pixels for overlap
N_WORKERS = 10
MIN_BOX_SIZE = 12
...
def generate_database(csv_files, img_files, output_folder, database_name, prefix):
    ...
    with multiprocessing.Pool(processes=N_WORKERS) as pool:
        ...
        stride = 200  # stride across the data, since we cannot fit all the results in memory at once
        for st_idx in range(0, len(csv_files), stride):
            print('{}/{}'.format(st_idx, len(csv_files)))
            tmp_csv_files = csv_files[st_idx:st_idx + stride]
